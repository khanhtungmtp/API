...
public static class ExcelUtility
{
    ...
    public static ExcelResult DownloadExcel<T>(List<T> data, string subPath, ConfigDownload configDownload)
    {
        ...
        {
            string errorMessage = ex.InnerException != null ? ex.InnerException.Message : "Unknown Error";
            return new ExcelResult(false, errorMessage);
        }
    }
    public static ExcelResult DownloadExcel(List<Table> dataTable, List<Cell> dataCell, string subPath, ConfigDownload configDownload)
    {
        configDownload ??= new ConfigDownload();
    ...
    public static ExcelResult DownloadExcel(List<Table> dataTable, string subPath, ConfigDownload configDownload)
    {
        configDownload ??= new ConfigDownload();
        if (!dataTable.Any())
            return new ExcelResult(false, "No data for excel download");
    ...
    public static ExcelResult DownloadExcel(List<Cell> dataCell, string subPath, ConfigDownload configDownload)
    {
        configDownload ??= new ConfigDownload();
        if (!dataCell.Any())
            return new ExcelResult(false, "No data for excel download");
...
public class Table(string root, object data)
{
